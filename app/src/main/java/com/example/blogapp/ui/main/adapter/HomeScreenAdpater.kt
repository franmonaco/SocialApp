...
class HomeScreenAdpater(private val postList: List<Post>, private val onPostClickListener: OnPostClickListener):
    ...
    private inner class HomeScreenViewHolder(
        ...
        private fun setupProfileInfo(post: Post) {
            Glide.with(context).load(post.poster?.profile_picture).into(binding.profilePicture)
            binding.profileName.text = post.poster?.profile_picture
        }
        ...
        private fun setupPostImage(post: Post){
            Glide.with(context).load(post.post_image).into(binding.postImage)
        }
        ...
        private fun tintHartIcon(post: Post){
            if(!post.liked) {
